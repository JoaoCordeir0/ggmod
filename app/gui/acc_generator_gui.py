...
def acc_render_gui():
    ...
    dpg.add_input_text(default_value="cordeiro", tag="input_user_acc")
    dpg.add_spacer(height=7)

    dpg.add_text("Senha das contas:")
    dpg.add_input_text(default_value="cordeiro321", tag="input_pass_acc")
    dpg.add_spacer(height=7)

    dpg.add_text("Quantidade de contas:")
    dpg.add_slider_int(default_value=1, max_value=1000, min_value=1, tag="input_amount_acc")
    dpg.add_spacer(height=7)

    with dpg.group(horizontal=True):
        dpg.add_text("Status:")
        status_acc_id = dpg.add_input_text(width=201, default_value="Não iniciado")

    with dpg.group(horizontal=True):
        generate_btn = dpg.add_button(label="Criar", width=70, callback=lambda: generate_accounts(status_acc_id))
